...
#django.views.genericからreverse_lazyをインポート
...
#デコレ―タ―により、CreatePhotoViewへのアクセスはログインユーザーに限定される
...
class CreatePhotoView(CreateView):
    ...
    '''
    #from.pyのPhotoPostFormをフォームクラスとして登録
    ...
    def form_valid(self, form):
        ...
        #投稿データをデータベースに登録
        postdata.save()
        #戻り値はスーパークラスのform_valid()の戻り値(HttpResponseRsdirect)
...
class PhotoDeleteView(DeleteView):
    ...
    Attributes:
        model:モデル
        template as_viewname:レンダリングするテンプレート
...
class PostUpdateView(UpdateView):
    ...
    #レンタリングするテンプレート
    template_name = "update_photo.html"
    success_url = reverse_lazy('photo:update_done')
    ...
    def form_valid(self, form):  
        ...
        # クリア時、画像なしの時image2_lim = False
        if postdata.image2 == '':
           # print("データなし")
            postdata.image2_lim = False
