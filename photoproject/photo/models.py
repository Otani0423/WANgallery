...
class PhotoPost(models.Model):
    ...
    max_length=20	  #最大文字数は200
    )
    #コメント用のフィールド
    comment = models.TextField(
    verbose_name='コメント',
    max_length=200	  #フィールドのタイトル
    )
    #イメージのフィールド1
    ...
    upload_to = 'photos'	   #MEDIA_ROOT以下のphotosにファイルを保存
      ...
      #イメージ1制限
    image1_lim = models.BooleanField(
    verbose_name='ダウンロード許可',  #フィールドのタイトル
    default=False
    )
    #イメージのフィールド2
    ...
    null=True		   #データベースにnullが保存されることを許容
     ...
     #イメージ2制限
    image2_lim = models.BooleanField(
    verbose_name='ダウンロード許可',  #フィールドのタイトル
    default=False
    )
    #投稿日時のフィールド
